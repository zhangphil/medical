...
public class TopDrawer extends VerticalDrawer {
	...
	public void setDropShadowColor(int color) {
		...
		mDropShadowDrawable = new GradientDrawable(
				GradientDrawable.Orientation.BOTTOM_TOP, new int[] { color,
						endColor, });
		invalidate();
	}
	...
	protected void drawDropShadow(Canvas canvas, int offsetPixels) {
		...
		mDropShadowDrawable.setBounds(0, offsetPixels - mDropShadowSize, width,
				offsetPixels);
	...
	protected void drawIndicator(Canvas canvas, int offsetPixels) {
		if (mActiveView != null && mActiveView.getParent() != null) {
			...
			if (pos == mActivePosition) {
				...
				canvas.save();
				canvas.clipRect(mIndicatorLeft, indicatorTop, mIndicatorLeft
						+ indicatorWidth, offsetPixels);
	...
	protected void onUpEvent(MotionEvent ev) {
		...
		if (mIsDragging) {
			...
			animateOffsetTo(
					mVelocityTracker.getYVelocity() > 0 ? mMenuSize : 0,

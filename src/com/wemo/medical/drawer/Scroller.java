...
public class Scroller {
	...
	private static final float DECELERATION_RATE = (float) (Math.log(0.75) / Math
			.log(0.9));
	private static final float ALPHA = 800; // pixels / seconds
	private static final float START_TENSION = 0.4f; // Tension at start: (0.4 *
														// total T, 1.0 *
														// Distance)
	...
	public boolean computeScrollOffset() {
		...
		if (timePassed < mDuration) {
			...
			case FLING_MODE:
				...
				final float distanceCoef = dInf + (t - tInf) / (tSup - tInf)
						* (dSup - dInf);

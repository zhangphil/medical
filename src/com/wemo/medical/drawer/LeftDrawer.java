...
public class LeftDrawer extends HorizontalDrawer {
	...
	public void setDropShadowColor(int color) {
		...
		mDropShadowDrawable = new GradientDrawable(
				GradientDrawable.Orientation.RIGHT_LEFT, new int[] { color,
						endColor, });
		invalidate();
	}
	...
	protected void drawDropShadow(Canvas canvas, int offsetPixels) {
		...
		mDropShadowDrawable.setBounds(offsetPixels - mDropShadowSize, 0,
				offsetPixels, height);
	...
	protected void drawIndicator(Canvas canvas, int offsetPixels) {
		if (mActiveView != null && mActiveView.getParent() != null) {
			...
			if (pos == mActivePosition) {
				...
				final int interpolatedWidth = (int) (mActiveIndicator
	...
	protected void onUpEvent(MotionEvent ev) {
		...
		if (mIsDragging) {
			...
			animateOffsetTo(
					mVelocityTracker.getXVelocity() > 0 ? mMenuSize : 0,

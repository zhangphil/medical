...
public class RightDrawer extends HorizontalDrawer {
	...
	public void setDropShadowColor(int color) {
		...
		mDropShadowDrawable = new GradientDrawable(
				GradientDrawable.Orientation.LEFT_RIGHT, new int[] { color,
						endColor, });
		invalidate();
	}
	...
	private void offsetMenu(int offsetPixels) {
		if (mOffsetMenu && mMenuSize != 0) {
			...
			final float openRatio = (menuWidth + (float) offsetPixels)
					/ menuWidth;
			...
			if (USE_TRANSLATIONS) {
				...
				final int newRight = width
						+ (int) (0.25f * (openRatio * menuWidth));
			...
			if (pos == mActivePosition) {
				...
				if (mIndicatorAnimating) {
					final int indicatorFinalTop = mActiveRect.top
							+ ((mActiveRect.height() - mActiveIndicator
									.getHeight()) / 2);
				...
				canvas.drawBitmap(mActiveIndicator, indicatorLeft,
						mIndicatorTop, null);
				canvas.restore();
			}
		}
	...
	protected void onMoveEvent(float dx) {
		final float newOffset = Math.max(Math.min(mOffsetPixels + dx, 0),
				-mMenuSize);
	...
	protected void onUpEvent(MotionEvent ev) {
		...
		if (mIsDragging) {
			...
			animateOffsetTo(mVelocityTracker.getXVelocity() > 0 ? 0
					: -mMenuSize, initialVelocity, true);

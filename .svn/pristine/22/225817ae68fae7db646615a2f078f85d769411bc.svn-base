...
public class BottomDrawer extends VerticalDrawer {
	...
	public void setDropShadowColor(int color) {
		...
		mDropShadowDrawable = new GradientDrawable(
				GradientDrawable.Orientation.TOP_BOTTOM, new int[] { color,
						endColor, });
		invalidate();
	}
	...
	private void offsetMenu(int offsetPixels) {
		if (mOffsetMenu && mMenuSize != 0) {
			...
			final int menuSize = mMenuSize;
			final float openRatio = (menuSize + (float) offsetPixels)
					/ menuSize;
	...
	protected void drawDropShadow(Canvas canvas, int offsetPixels) {
		...
		mDropShadowDrawable.setBounds(0, height + offsetPixels, width, height
				+ offsetPixels + mDropShadowSize);
	...
	protected void drawIndicator(Canvas canvas, int offsetPixels) {
		if (mActiveView != null && mActiveView.getParent() != null) {
			...
			if (pos == mActivePosition) {
				...
				final float openRatio = ((float) Math.abs(offsetPixels))
						/ menuHeight;
				...
				if (mIndicatorAnimating) {
					...
					mIndicatorLeft = mActiveRect.left
							+ ((mActiveRect.width() - indicatorWidth) / 2);
				}

				canvas.save();
				canvas.clipRect(mIndicatorLeft, height + offsetPixels,
						mIndicatorLeft + indicatorWidth, indicatorBottom);
				canvas.drawBitmap(mActiveIndicator, mIndicatorLeft,
						indicatorTop, null);
				canvas.restore();
			}
		}
	...
	protected void onUpEvent(MotionEvent ev) {
		...
		if (mIsDragging) {
			...
			animateOffsetTo(mVelocityTracker.getYVelocity() < 0 ? -mMenuSize
					: 0, initialVelocity, true);
